...
/**
 * Created by 徐启鑫 on 2017/12/6.
 */

public class Qianjiaozhang1 extends View {
    //坐标轴原点的位置
    ...
    private Handler mh=new Handler(){
        public void handleMessage(android.os.Message msg) {
            if(msg.what==0){                //判断接受消息类型
                Qianjiaozhang1.this.invalidate();  //刷新View
            }
        };
    };
    public Qianjiaozhang1(Context context, AttributeSet attrs) {
        super(context, attrs);
            ...
            public void run() {
                while(true){     //在线程中不断往集合中增加数据
                    ...
                    if (MainActivity.m_receive_data_down!=null) {
                        v = MainActivity.m_receive_data_down[21]+
...
//        canvas.drawLine(xPoint, yPoint, xPoint+xLength, yPoint, paint);
        ...
        if(data.size()>1){
            for (int i = 1; i < data.size()-1; i++) {  //依次取出数据进行绘制
                canvas.drawLine(xPoint+(i-1)*xScale,(float)( yPoint-((data.get(i-1)))*yScale-yScale), xPoint+i*xScale, (float)(yPoint-(data.get(i))*yScale-yScale), paint);
            }
        }
    }
}
